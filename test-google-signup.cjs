// Google Sign-Up Test Script (CommonJS)
...
async function testGoogleSignUp() {
  ...
  try {
    ...
    const { data: oauthData, error: oauthError } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: process.env.OAUTH_REDIRECT_TO || 'http://localhost:5173/auth/callback',
    ...
    if (oauthError) {
      ...
      if (oauthError.message && oauthError.message.includes('redirect_uri_mismatch')) {
        ...
        console.error('And also add your app callback, e.g.: http://localhost:5173/auth/callback');
      }
...
async function runAllTests() {
  ...
  try {
    ...
    if (results.googleOAuth && results.database) {
      ...
      console.log(process.env.SITE_URL || 'http://localhost:5173/register');
    } else {
