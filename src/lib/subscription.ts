/**
 * Practitioner subscriptions + Stripe Customer Portal (app parity).
 */
...
export async function fetchLatestSubscription(
  ...
  try {
    ...
    if (!data) {
      return { data: { subscription: null, accessDenied: false }, error: null };
    }
...
export async function createCustomerPortalSession(): Promise<CustomerPortalResult> {
  ...
  if (!url || typeof url !== "string") {
    return {
      success: false,
      error:
        "Billing portal did not return a URL. Try again or contact support.",
    };
  }
