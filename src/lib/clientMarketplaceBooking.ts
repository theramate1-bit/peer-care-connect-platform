...
import { createSessionCheckout } from "@/lib/paymentSessionCheckout";
...
export type FetchPractitionerProductsOptions = {
  /** Exclude mobile-only services (clinic / web marketplace booking). Matches native Explore booking. */
  clinicBooking?: boolean;
};

export async function fetchPractitionerProducts(
  practitionerId: string,
  options?: FetchPractitionerProductsOptions,
): Promise<{
  ...
  try {
    ...
    let rows = (data || []) as PractitionerProductRow[];
    if (options?.clinicBooking) {
      rows = rows.filter(
        (r) =>
          !r.service_type ||
          r.service_type === "clinic" ||
          r.service_type === "both",
      );
    }
...
/** Keep aligned with `theramate-ios-client/constants/config.ts` BOOKING_CONFIG.MIN_BOOKING_BUFFER_HOURS */
...
/** Calendar YYYY-MM-DD in IANA `timeZone` for instant `d`. */
...
/**
 * UTC epoch ms for local wall-clock `timeHHMM` on calendar `dateStr` in `timeZone`.
 * Scans in 1-minute steps (DST-safe for practical slot grids).
 */
...
/** Weekday 0–6 (Sun–Sat) for `dateStr` interpreted in practitioner's IANA zone. */
...
export async function fetchAvailableStartTimes(params: {
  ...
  try {
    ...
    const cutoff = Date.now() + MIN_BOOKING_BUFFER_HOURS * 3600 * 1000;
...
export type BookAndPayParams = {
  ...
  notes?: string | null;
  /** Guest marketplace bookings (native parity). */
...
export async function bookSessionAndOpenCheckout(
  ...
  const { data: bookingResult, error: rpcError } = await supabase.rpc(
    "create_booking_with_validation",
    {
      ...
      p_is_guest_booking: p.isGuestBooking ?? false,
  ...
  const payment = await createSessionCheckout({
    ...
    practitionerName: p.practitionerName?.trim() || "Practitioner",
