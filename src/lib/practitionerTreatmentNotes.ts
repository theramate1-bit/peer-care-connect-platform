...
import { fetchClinicalAttachmentCountsBySession } from "@/lib/clinicalSessionAttachments";
import {
...
export function notesByType(rows: TreatmentNoteRow[]): Record<string, string> {
...
export async function fetchSessionsClinicalNotesSummary(params: {
  ...
  try {
    const { data: sessions, error } = await fetchPractitionerSessions(
      params.therapistId,
    );
    if (error) throw error;
    ...
    const bySession = new Map<
      string,
      { has: boolean; maxUpdatedAt: string | null }
    >();
    for (const row of (notes || []) as {
      session_id: string | null;
      updated_at: string | null;
    }[]) {
      ...
      if (!prev) {
        bySession.set(row.session_id, {
          has: true,
          maxUpdatedAt: row.updated_at ?? null,
        });
        continue;
      }
