...
export type PractitionerDashboardData = {
  ...
  exchangeOutgoingPendingCount: number;
...
export async function fetchPractitionerDashboard(
  ...
  try {
    const [
      ...
      fetchPendingExchangeRequestsSentByRequester(practitionerId),
    ...
    if (exchangeSentRes.error) throw exchangeSentRes.error;
    ...
    const data: PractitionerDashboardData = {
      ...
      exchangeOutgoingPendingCount: exchangeSentRes.data.length,
