...
export const CLINICAL_SESSION_ATTACHMENTS_BUCKET =
  "clinical-session-attachments";
...
export async function fetchClinicalSessionAttachments(
  ...
  try {
    ...
    if (error) throw error;
    return {
      data: (data || []) as ClinicalSessionAttachmentRow[],
      error: null,
    };
  } catch (e) {
    return {
...
/** Browser file upload (web parity). */
export async function uploadClinicalSessionFile(params: {
  sessionId: string;
  ...
  const safe =
    params.file.name.replace(/[^a-zA-Z0-9._-]+/g, "_").slice(0, 120) || "file";
