-- Migration: Fix RETURNING clause in RPC function
-- Created: 2025-12-23
-- Description: Use column name only in RETURNING, not table.column

-- Fix the RETURNING clause - use column name only, not table.column
-- The ambiguity error is likely from the RETURN QUERY SELECT
...
BEGIN
  ...
  -- In RETURNING clause, use column name only (not table.column)
  ...
  -- Return the full slot hold record - use table alias to avoid any ambiguity
  RETURN QUERY
  SELECT 
    sh.id,
    sh.practitioner_id,
