-- Create process_peer_booking_credits RPC function
-- This is a CRITICAL function that was missing from the system
-- It handles atomic credit transactions for peer treatment bookings
...
BEGIN
    -- Get credit cost for this session
    ...
    IF v_practitioner_balance IS NULL THEN
        ...
        ON CONFLICT (user_id) DO UPDATE
        SET balance = EXCLUDED.balance
        RETURNING balance, total_earned INTO v_practitioner_balance, v_practitioner_earned;
    END IF;
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION process_peer_booking_credits(UUID, UUID, UUID, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION process_peer_booking_credits(UUID, UUID, UUID, INTEGER) TO service_role;

-- Add helpful comment
COMMENT ON FUNCTION process_peer_booking_credits IS 
'Atomically processes credit transaction for peer treatment bookings. Validates balance, deducts from client, awards to practitioner. Uses row-level locking (FOR UPDATE) to prevent race conditions. Returns JSON with success status and credit details.';
