-- Update process_peer_booking_credits to accept product_id parameter
-- Passes product_id to get_practitioner_credit_cost for accurate credit calculation
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION process_peer_booking_credits(UUID, UUID, UUID, INTEGER, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION process_peer_booking_credits(UUID, UUID, UUID, INTEGER, UUID) TO service_role;

-- Update comment
COMMENT ON FUNCTION process_peer_booking_credits IS 
'Atomically processes credit transaction for peer treatment bookings using practitioner_products. Validates balance, deducts from client, awards to practitioner. Uses row-level locking (FOR UPDATE) to prevent race conditions. Returns JSON with success status and credit details.';
