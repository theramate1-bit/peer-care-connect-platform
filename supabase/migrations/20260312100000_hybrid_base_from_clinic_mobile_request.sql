-- Hybrid practitioners: base location is derived from clinic. Use clinic_latitude/clinic_longitude
-- when base_* is null so mobile booking request and distance checks work without a separate base address.
...
begin
  select id, therapist_type, base_latitude, base_longitude, clinic_latitude, clinic_longitude,
  ...
  -- Origin for distance: base for mobile; for hybrid use base if set, else clinic (base derived from clinic).
  v_origin_lat := case
    when v_practitioner.therapist_type = 'hybrid' then coalesce(v_practitioner.base_latitude, v_practitioner.clinic_latitude)
    else v_practitioner.base_latitude
  end;
  v_origin_lon := case
    when v_practitioner.therapist_type = 'hybrid' then coalesce(v_practitioner.base_longitude, v_practitioner.clinic_longitude)
    else v_practitioner.base_longitude
  end;
  ...
  v_distance_km := st_distance(
    ...
    st_setsrid(st_makepoint(v_origin_lon, v_origin_lat), 4326)::geography
  ) / 1000.0;
...
comment on function public.create_mobile_booking_request(uuid, uuid, uuid, date, time, integer, text, numeric, numeric, text, jsonb) is
  'Creates a mobile booking request. For hybrid practitioners, base location is derived from clinic when base_* is null.';
