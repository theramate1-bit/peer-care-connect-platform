-- P1-1: After mobile request accept, queue confirmation emails with visit address (pg_net → send-booking-notification).

CREATE OR REPLACE FUNCTION public.queue_mobile_accept_confirmation_emails(
  p_session_id uuid,
  p_request_id uuid
)
RETURNS void
...
BEGIN
  ...
  IF v_service_role_key IS NULL OR trim(v_service_role_key) = '' THEN
    RAISE WARNING 'queue_mobile_accept_confirmation_emails: service_role_key not configured';
    RETURN;
  END IF;
  ...
  PERFORM net.http_post(
    ...
    body := jsonb_build_object(
      ...
      'emailType', 'mobile_accept'
    )
  );
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'queue_mobile_accept_confirmation_emails failed for session %: %', p_session_id, SQLERRM;
END;
$function$;

-- accept_mobile_booking_request: queue emails after successful session creation (non-blocking).
...
BEGIN
  ...
  PERFORM public.queue_mobile_accept_confirmation_emails(v_session_id, p_request_id);
