-- Add guest_view_token to client_sessions for token-based "View Booking Details" links in emails.
-- Token is set when payment is completed (Stripe webhook); guests can view booking without logging in.
ALTER TABLE public.client_sessions
  ADD COLUMN IF NOT EXISTS guest_view_token TEXT;

CREATE UNIQUE INDEX IF NOT EXISTS idx_client_sessions_guest_view_token
  ON public.client_sessions(guest_view_token)
  WHERE guest_view_token IS NOT NULL;

COMMENT ON COLUMN public.client_sessions.guest_view_token IS 'Secure token for guest view link in booking confirmation email; used in /booking/view/:sessionId?token=...';

-- RPC: return session + practitioner public details when token matches (no auth required).
...
COMMENT ON FUNCTION public.get_session_by_guest_token(UUID, TEXT) IS 'Returns public session details for guest view page when token matches; used by /booking/view route.';
