-- Reduce pending_payment hold time from 60 minutes to 15 minutes
-- This prevents abandoned bookings from blocking slots for too long

-- Update the trigger function to use 15 minutes instead of 60
...
AS $$
BEGIN
  -- Auto-populate expires_at for pending_payment holds (reduced to 15 minutes)
...
-- Update the create_booking_with_validation function to use 15 minutes
-- (This is used in the main booking flow)
...
BEGIN
  ...
  -- Create the booking (using 15 minutes hold time instead of 60)
  ...
  VALUES (
    ...
    COALESCE(p_expires_at, CASE WHEN p_status = 'pending_payment' THEN NOW() + INTERVAL '15 minutes' ELSE NULL END),
    p_idempotency_key
  )
...
COMMENT ON FUNCTION public.create_booking_with_validation IS 'Creates a booking with comprehensive validation and 15-minute hold time for pending payments';
