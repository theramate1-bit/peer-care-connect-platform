-- Mobile bookings page: show most recent first (created_at DESC) instead of soonest-scheduled first.
-- Aligns practitioner view with user expectation: newest requests at top.
...
BEGIN
  ...
  FROM ranked
  WHERE ranked.rn = 1
  ORDER BY ranked.created_at DESC, ranked.requested_date ASC, ranked.requested_start_time ASC;
END;
$function$;
