...
serve(async (req) => {
  ...
  try {
    ...
    if (!body?.audio_url && !body?.storage_path) {
      return new Response(
        JSON.stringify({ error: "audio_url or storage_path is required" }),
        {
          status: 400,
          headers: {
            ...corsHeaders(origin),
    ...
    if (body.storage_path && typeof body.storage_path === "string") {
      if (body.storage_path.length > 500 || body.storage_path.includes("..")) {
        return new Response(
          JSON.stringify({ error: "Invalid storage_path format" }),
          {
            status: 400,
            headers: {
              ...corsHeaders(origin),
    ...
    const hashSource = body.storage_path || body.audio_url || submitAudioUrl;
    await logAiProcessing(supabase, {
      userId: user.id,
      functionName: "transcribe-file",
      subProcessor: "assemblyai",
      modelId: ASSEMBLYAI_MODEL_ID,
      inputType: body.storage_path ? "storage_path" : "audio_url",
      inputForHash: hashSource,
