...
Deno.serve(async (req) => {
  // Handle CORS preflight first (so it always returns CORS headers even if gateway doesn't)
  ...
  try {
    ...
    if (!Deno.env.get("STRIPE_SECRET_KEY")) {
      console.error(
        "[STRIPE-PAYMENT] Missing STRIPE_SECRET_KEY environment variable",
      );
      return new Response(
        JSON.stringify({
    ...
    switch (method) {
      case "POST":
        ...
        } else if (action === "create-mobile-checkout-session") {
          return await handleCreateMobileCheckoutSession(
            req,
            body,
            supabaseAdmin,
          );
        } else if (action === "confirm-mobile-checkout-session") {
          return await handleConfirmMobileCheckoutSession(
            req,
            body,
            supabaseAdmin,
          );
        } else if (action === "capture-mobile-payment") {
...
async function handleCreatePaymentIntent(
  ...
  try {
    ...
    if (!amount || !currency) {
      console.error("[CREATE-PAYMENT] Missing required fields:", {
        amount,
        currency,
      });
      return new Response(
    ...
    if (!session_id || session_id.trim() === "") {
      console.error("[CREATE-PAYMENT] Missing session_id:", {
        session_id,
        payment_type,
      });
      return new Response(
    ...
    if (
      ...
      typeof metadata.client_user_id !== "string"
    ) {
      return new Response(
        JSON.stringify({
    ...
    if (idempotency_key) {
      ...
      if (existingPayment) {
        console.log(
          "[CREATE-PAYMENT] Found existing payment with idempotency key:",
          idempotency_key,
        );
        ...
        return new Response(
          JSON.stringify({
            ...
            customer_ephemeral_key_secret:
              existingMetadata.customer_ephemeral_key_secret || null,
            payment_intent_client_secret:
              existingMetadata.payment_intent_client_secret || null,
          }),
          {
            status: 200,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    // Create/reuse a Stripe customer so mobile PaymentSheet can be enabled when available.
    let stripeCustomerId: string | null = null;
    let customerEphemeralKeySecret: string | null = null;
    try {
      const customerEmail = metadata.client_email || undefined;
      if (customerEmail) {
        const customerSearch = await stripe.customers.list({
          email: customerEmail,
          limit: 1,
        });
        const existingCustomer = customerSearch.data?.[0] || null;
        const customer =
          existingCustomer ||
          (await stripe.customers.create({
            email: customerEmail,
            name: metadata.client_name || undefined,
            metadata: {
        ...
        customerEphemeralKeySecret = ephemeralKey.secret || null;
      }
    } catch (customerError) {
      // Non-fatal: checkout flow can still proceed without these fields.
      console.warn(
        "[CREATE-PAYMENT] Customer/ephemeral key setup skipped:",
...
async function handleCreateConnectAccount(
  ...
  try {
    ...
    console.log("[CREATE-CONNECT] Starting with params:", {
      userId,
      email,
      firstName,
      lastName,
    ...
    if (userCheckError || !publicUser) {
      console.error("[CREATE-CONNECT] User not found in public.users:", {
        authenticatedUserId,
        userCheckError,
      });
      return new Response(
        JSON.stringify({
          ...
          details:
            "User must exist in public.users before creating Connect account",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (fetchError) {
      console.error(
        "[CREATE-CONNECT] Error checking existing account:",
        fetchError,
      );
    }
    ...
    if (existingAccount && existingAccount.stripe_account_id) {
      ...
      console.log(
        "[CREATE-CONNECT] Existing account found:",
      ...
      try {
        ...
        if (!retrieveResponse.ok) {
          ...
          console.warn(
            "[CREATE-CONNECT] Existing account cannot be retrieved, will create new account",
          );
          await supabase
            .from("connect_accounts")
            .delete()
          ...
          const cardPaymentsRequested =
            merchant.capabilities?.card_payments?.requested === true;
          const stripeTransfersRequested =
          ...
          if (!cardPaymentsRequested || !stripeTransfersRequested) {
            console.log(
              "[CREATE-CONNECT] Missing capabilities, updating account...",
            );
            ...
            if (updateResponse.ok) {
              console.log(
                "[CREATE-CONNECT] Successfully updated account capabilities",
              );
              ...
              console.error(
                "[CREATE-CONNECT] Failed to update account capabilities:",
          ...
          if (!isV2Account || !hasCorrectConfig) {
            console.warn(
              "[CREATE-CONNECT] Existing account has incorrect configuration:",
              {
                dashboard,
                fees_collector: feesCollector,
                losses_collector: lossesCollector,
                isFullyOnboarded,
                reason: !isV2Account
                  ? 'Not Accounts v2 (dashboard !== "none")'
                  : 'Responsibilities not set to "application"',
              },
            );
            ...
            if (isFullyOnboarded) {
              console.warn(
                "[CREATE-CONNECT] Account is fully onboarded - cannot migrate. Will use existing account.",
              );
            ...
            console.log(
              "[CREATE-CONNECT] Deleted account with incorrect config, will create new Accounts v2 account",
            );
            ...
            return new Response(
              JSON.stringify({
                ...
                status: "pending",
                message:
                  "Accounts v2 account exists, ready for embedded onboarding",
              }),
              {
                status: 200,
                headers: { ...corsHeaders, "Content-Type": "application/json" },
        ...
        await supabase
          ...
          .eq("stripe_account_id", existingAccount.stripe_account_id);
        console.log(
          "[CREATE-CONNECT] Deleted unverifiable account, will create new Accounts v2 account",
        );
      }
    }
    ...
    if (!accountId) {
      console.log(
        "[CREATE-CONNECT] Creating new account using Accounts v2 API (official way, no fallbacks)",
      );
      ...
      const registeredName =
        businessType === "company" ? displayName : displayName;
      ...
      try {
          ...
          "https://api.stripe.com/v2/core/accounts",
          {
            ...
            body: JSON.stringify({
              ...
              identity: {
                ...
                entity_type:
                  businessType === "company" ? "company" : "individual",
              },
              configuration: {
        ...
        if (!v2Response.ok) {
          ...
          return new Response(
            JSON.stringify({
              ...
              details:
                "Accounts v2 API request failed. Only Accounts v2 is supported (no fallbacks).",
        ...
        console.log(
          "[CREATE-CONNECT] Created Accounts v2 account (official way):",
          accountId,
          {
...
async function handleGetConnectAccountStatus(
  req: Request,
  body: any,
  supabase: any,
) {
  try {
    ...
    // Query by stripe_account_id first to avoid duplicate-account drift.
    let connectAccount;
    let queryMethod = "none";

    if (finalAccountId) {
      console.log(
        "[CONNECT-STATUS] Querying by stripe_account_id:",
        finalAccountId,
      );
      ...
      if (accountError) {
        console.error(
          "[CONNECT-STATUS] Error querying by stripe_account_id:",
          accountError,
        );
        ...
        console.log(
          "[CONNECT-STATUS] Found record by stripe_account_id:",
          connectAccount.id,
        );
      }
    }
    ...
    if (!connectAccount && finalUserId) {
      console.log(
        "[CONNECT-STATUS] Fallback: Querying by user_id:",
        finalUserId,
      );
      ...
      if (userError) {
        ...
        queryMethod = "user_id";
        console.log(
          "[CONNECT-STATUS] Found record by user_id:",
          connectAccount.id,
        );
        ...
        if (!countError && count && count > 1) {
          console.warn(
            `[CONNECT-STATUS] WARNING: User has ${count} connect_accounts records. Using most recent.`,
          );
          console.warn(
            `[CONNECT-STATUS] User's stripe_connect_account_id: ${finalAccountId}, Selected record: ${connectAccount.stripe_account_id}`,
          );
        }
      }
    }
    ...
    if (finalAccount) {
      ...
      console.log(
        "[CONNECT-STATUS] Database stripe_account_id:",
        finalAccount.stripe_account_id,
      );
    }
...
async function handleVerifyConnectAccount(
  req: Request,
  body: any,
  supabase: any,
) {
  try {
    ...
    if (!targetAccountId && !targetUserId) {
      const errorResponse = {
        ...
        message:
          "Either therapist_connect_account_id or practitioner_id/user_id must be provided",
      };

      console.error(
        "[VERIFY-CONNECT-ACCOUNT] Validation failed:",
        errorResponse,
      );
...
async function handleCreateCheckoutSession(
  ...
  try {
    ...
    if (!practitioner_id || !price_id) {
      ...
      return new Response(
        JSON.stringify({
          error: "Missing required fields: practitioner_id, price_id",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (practitionerError || !practitioner?.stripe_connect_account_id) {
      console.log(
        "[CREATE-CHECKOUT-SESSION] Practitioner not found or not connected:",
        practitionerError,
      );
      return new Response(
    ...
    if (dbError) {
      console.error(
        "[CREATE-CHECKOUT-SESSION] Database error (non-fatal):",
        dbError,
      );
...
async function handleCreateMobileCheckoutSession(
  req: Request,
  body: any,
  supabase: any,
) {
  try {
    ...
    if (mobileRequest.status !== "pending") {
      return new Response(
        JSON.stringify({
          error: `Mobile request is not pending (status: ${mobileRequest.status})`,
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (Number(amount) !== Number(mobileRequest.total_price_pence)) {
      return new Response(
        JSON.stringify({
          error: "Amount mismatch for mobile request",
          details: {
            expected: mobileRequest.total_price_pence,
            received: amount,
          },
        }),
        {
    ...
    let destinationAccountId = therapist_connect_account_id;
    if (!destinationAccountId) {
      const { data: practitioner } = await supabase
        ...
        .eq("id", practitioner_id)
        .maybeSingle();
      destinationAccountId = practitioner?.stripe_connect_account_id;
    }
    ...
    const paymentMetadata = {
      ...
      request_id,
      practitioner_id,
      product_id: mobileRequest.product_id || "",
      client_user_id: mobileRequest.client_id || metadata?.client_user_id || "",
      destination_account_id: destinationAccountId,
      ...metadata,
    };

    const baseCheckoutPayload: any = {
      mode: "payment",
      payment_method_types: ["card"],
      customer_email: client_email,
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: metadata?.service_name || "Mobile Session",
              description: `Mobile booking request ${request_id}`,
            },
          },
          quantity: 1,
        },
    ...
    try {
      ...
      console.warn(
        "[CREATE-MOBILE-CHECKOUT-SESSION] Destination transfer capability missing; retrying without transfer_data",
        {
          request_id,
          practitioner_id,
          destination_account_id: destinationAccountId,
    ...
    console.error("[CREATE-MOBILE-CHECKOUT-SESSION] Error:", error);
    return new Response(
      JSON.stringify({
...
async function handleConfirmMobileCheckoutSession(
  req: Request,
  body: any,
  supabase: any,
) {
  try {
    ...
    if (
      ...
      return new Response(
        JSON.stringify({
          error: "Checkout session does not belong to request",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (checkoutSession.payment_status !== "paid") {
      return new Response(
        JSON.stringify({
          ...
          details: { payment_status: checkoutSession.payment_status },
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (!paymentIntent?.id) {
      return new Response(
        JSON.stringify({
          error: "Payment intent not found for checkout session",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.error("[CONFIRM-MOBILE-CHECKOUT-SESSION] Error:", error);
    return new Response(
      JSON.stringify({
        error: "Failed to confirm mobile checkout session",
        details: error?.message || String(error),
      }),
      {
        status: 500,
...
async function handleCaptureMobilePayment(
  req: Request,
  body: any,
  supabase: any,
) {
  try {
    ...
    if (paymentIntent.status === "requires_capture") {
      ...
      return new Response(
        JSON.stringify({
          error: `Payment intent cannot be captured from status ${paymentIntent.status}`,
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    // If capture succeeded without transfer_data, perform platform -> connect transfer now.
    const transferDestination =
      paymentIntent.transfer_data?.destination || null;
    if (finalStatus === "succeeded" && !transferDestination) {
      try {
        const { data: requestRow } = await supabase
          .from("mobile_booking_requests")
          .select("id, practitioner_id, total_price_pence, platform_fee_pence")
          .eq("stripe_payment_intent_id", payment_intent_id)
          .maybeSingle();

        if (requestRow?.practitioner_id) {
          const { data: practitioner } = await supabase
            .from("users")
            .select("stripe_connect_account_id")
          ...
          const destinationAccountId = practitioner?.stripe_connect_account_id;
          if (destinationAccountId) {
            const grossAmount = Number(
              requestRow.total_price_pence ??
            ...
            if (practitionerAmount > 0) {
              ...
              const transfer = await stripe.transfers.create({
                ...
                transfer_group: requestRow.id
                  ? `mobile_request_${requestRow.id}`
                  : undefined,
                metadata: {
                  payment_intent_id,
                  request_id: requestRow.id || "",
                  practitioner_id: requestRow.practitioner_id || "",
                  payout_flow: "mobile_capture_fallback",
                },
              });
            ...
            transferWarning =
              "Payment captured, but practitioner has no connected destination account for transfer.";
          }
        }
      } catch (transferError: any) {
        console.error(
          "[CAPTURE-MOBILE-PAYMENT] Deferred transfer failed:",
          transferError,
        );
        transferWarning =
          transferError?.message ||
          "Payment captured, but transfer to practitioner could not be completed automatically.";
      }
    }
    ...
    console.error("[CAPTURE-MOBILE-PAYMENT] Error:", error);
    return new Response(
      JSON.stringify({
        error: "Failed to capture mobile payment",
        details: error?.message || String(error),
      }),
      {
        status: 500,
...
async function handleReleaseMobilePayment(
  req: Request,
  body: any,
  supabase: any,
) {
  try {
    ...
    if (
      ...
      return new Response(
        JSON.stringify({
          error: `Payment intent cannot be released from status ${paymentIntent.status}`,
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.error("[RELEASE-MOBILE-PAYMENT] Error:", error);
    return new Response(
      JSON.stringify({
        error: "Failed to release mobile payment",
        details: error?.message || String(error),
      }),
      {
        status: 500,
...
async function handleCreateProduct(req: Request, body: any, supabase: any) {
  try {
    ...
    const serviceType = ["clinic", "mobile", "both"].includes(rawServiceType)
      ? rawServiceType
      : "clinic";
    ...
    const amount =
      typeof price_amount === "number"
        ? Math.round(price_amount)
        : parseInt(String(price_amount), 10);
    ...
    const dur =
      duration_minutes != null ? parseInt(String(duration_minutes), 10) : 60;
    ...
    if (!practitioner?.stripe_connect_account_id) {
      return new Response(
        JSON.stringify({
          ...
          message:
            "Please complete Stripe Connect onboarding before creating products",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (dbError) {
      console.error("[CREATE-PRODUCT] DB insert error:", {
...
/** Allowed columns for practitioner_products updates (excludes request-only fields like action, product_id) */
...
async function handleUpdateProduct(req: Request, body: any, supabase: any) {
  try {
    ...
    for (const key of PRACTITIONER_PRODUCTS_UPDATE_KEYS) {
    ...
    if (
      !existingProduct ||
      authData.user.id !== existingProduct.practitioner_id
    ) {
      return new Response(JSON.stringify({ error: "Forbidden" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const { data: practitioner } = await supabase
      .from("users")
      .select("stripe_connect_account_id")
      .eq("id", existingProduct.practitioner_id)
      .single();
    ...
    if (!practitioner?.stripe_connect_account_id) {
      return new Response(
        JSON.stringify({
          ...
          requires_connect: true,
          message:
            "Please complete Stripe Connect onboarding before updating products",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (
      updates.service_category !== undefined ||
      ...
      const updatedMetadata: Record<string, string> = {
        ...
        duration_minutes:
          (
            updates.duration_minutes ?? existingProduct.duration_minutes
          )?.toString() || "",
        category: updates.category || existingProduct.category || "general",
        service_category:
          updates.service_category || existingProduct.service_category || "",
      };
...
async function handleCreateAccountSession(
  ...
  try {
    ...
    console.log("[CREATE-ACCOUNT-SESSION] Starting with params:", {
      stripe_account_id,
      components,
    });
    ...
    if (!accountId) {
      ...
      if (fetchError || !connectAccount?.stripe_account_id) {
        console.error(
          "[CREATE-ACCOUNT-SESSION] No Connect account found for user:",
          authenticatedUserId,
        );
        return new Response(
          JSON.stringify({
      ...
      if (
        verifyError ||
        !verifyAccount ||
        verifyAccount.user_id !== authenticatedUserId
      ) {
        console.error(
          "[CREATE-ACCOUNT-SESSION] User does not own this account:",
          { accountId, authenticatedUserId },
        );
        return new Response(
          JSON.stringify({
            error: "Unauthorized: you do not own this Stripe Connect account",
          }),
          {
            status: 403,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.log(
      "[CREATE-ACCOUNT-SESSION] Creating session for account:",
      accountId,
    );
    ...
    try {
      ...
      if (accountDashboard !== "none") {
        console.error(
          "[CREATE-ACCOUNT-SESSION] Account is not Accounts v2 format:",
          accountDashboard,
        );
        return new Response(
          JSON.stringify({
    ...
    if (platformOwnsRequirements) {
      ...
      console.log(
        "[CREATE-ACCOUNT-SESSION] Account has platform-owned requirements - enabling disable_stripe_user_authentication",
      );
    } else {
      console.log(
        "[CREATE-ACCOUNT-SESSION] Account does not have platform-owned requirements - NOT enabling disable_stripe_user_authentication",
      );
      if (
        ...
        console.log(
          "[CREATE-ACCOUNT-SESSION] Removing disable_stripe_user_authentication from frontend components (incompatible with account config)",
        );
      }
    }
    ...
    if (
      ...
      finalFeatures.disable_stripe_user_authentication
    ) {
      payoutsFeatures.disable_stripe_user_authentication = true;
      console.log(
        "[CREATE-ACCOUNT-SESSION] Setting disable_stripe_user_authentication for payouts to match account_onboarding",
      );
    } else {
      delete payoutsFeatures.disable_stripe_user_authentication;
      if (frontendPayoutsFeatures.disable_stripe_user_authentication) {
        console.log(
          "[CREATE-ACCOUNT-SESSION] Removed disable_stripe_user_authentication from payouts (account does not support it)",
        );
      }
    }
    ...
    const componentsConfig = {
      ...
      account_management: {
        enabled: components?.account_management?.enabled ?? true,
      },
      payouts: {
        enabled: components?.payouts?.enabled ?? true,
        features:
          Object.keys(payoutsFeatures).length > 0 ? payoutsFeatures : undefined,
      },
      payments: { enabled: components?.payments?.enabled ?? true },
      notification_banner: {
        enabled: components?.notification_banner?.enabled ?? true,
      },
    ...
    console.log(
      "[CREATE-ACCOUNT-SESSION] Using fully embedded configuration for Accounts v2 account",
    );

    console.log(
      "[CREATE-ACCOUNT-SESSION] Components config:",
    ...
    try {
      ...
      console.error(
        "[CREATE-ACCOUNT-SESSION] Stripe accountSessions.create failed:",
        {
