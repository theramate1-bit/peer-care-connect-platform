...
8. **Universal Links** — `associatedDomains` are set for `theramate.co.uk` / `www.theramate.co.uk` (see `app.config.js`). Static files live in `peer-care-connect/public/.well-known/` (`apple-app-site-association`, `assetlinks.json`) with JSON `Content-Type` via `public/_headers`. After deploy, run `UNIVERSAL_LINKS_PROBE=1 npm run check:universal-links`. For Android, add the EAS signing cert SHA-256 to `assetlinks.json` (`eas credentials -p android`).
