/**
 * Practitioner-facing session status labels — keep aligned with
 * `peer-care-connect/src/lib/session-display-status.ts`.
 */
...
export function isSuccessfulSessionPayment(
...
export function getDisplaySessionStatus(
  ...
  if (
    ...
    (normalizedStatus === "pending_payment" || normalizedStatus === "scheduled")
  ) {
    return "confirmed";
  }
