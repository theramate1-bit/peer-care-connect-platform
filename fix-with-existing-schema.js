// Fix script using only existing table columns
...
async function fixWithExistingSchema() {
  console.log('🔧 Fixing user data with existing schema...\n');

  try {
    // Step 1: Update user profile (only existing columns)
    console.log('1. Updating user profile...');
    const { error: userUpdateError } = await supabase
      .from('user_profiles')
      .update({
        last_name: 'Doe' // Only update the last_name field that exists
      })
      .eq('id', userId);
