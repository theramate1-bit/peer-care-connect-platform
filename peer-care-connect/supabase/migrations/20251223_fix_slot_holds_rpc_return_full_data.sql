-- Migration: Update RPC function to return full slot hold record
-- Created: 2025-12-23
-- Description: Return full slot hold data from RPC to avoid RLS issues on SELECT

-- Drop and recreate the function to return the full slot hold record
-- This avoids needing a separate SELECT query that might be blocked by RLS
...
BEGIN
  ...
  -- Return the full slot hold record
  RETURN QUERY
  SELECT 
    sh.id,
    sh.practitioner_id,
