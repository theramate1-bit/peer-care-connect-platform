-- Fix process_peer_booking_refund to validate credits were actually paid
...
'Atomically processes credit refund for cancelled peer treatment sessions. Validates that credits were actually paid before refunding. Returns credits to client, deducts from practitioner only if they earned credits. Uses row-level locking to prevent race conditions.';
