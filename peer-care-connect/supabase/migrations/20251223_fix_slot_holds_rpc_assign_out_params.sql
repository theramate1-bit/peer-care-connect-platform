-- Migration: Assign directly to OUT parameters to avoid ambiguity
-- Created: 2025-12-23
-- Description: Assign RECORD fields to OUT parameters, then RETURN NEXT

-- Assign directly to OUT parameters (RETURNS TABLE creates OUT parameters)
-- This avoids the ambiguity issue completely
...
BEGIN
  ...
  -- Assign directly to OUT parameters (RETURNS TABLE creates OUT parameters)
  -- This avoids any ambiguity with column names
  id := v_slot_hold.id;
  practitioner_id := v_slot_hold.practitioner_id;
  request_id := v_slot_hold.request_id;
  session_date := v_slot_hold.session_date;
  start_time := v_slot_hold.start_time;
  end_time := v_slot_hold.end_time;
  duration_minutes := v_slot_hold.duration_minutes;
  expires_at := v_slot_hold.expires_at;
  status := v_slot_hold.status;
  created_at := v_slot_hold.created_at;
  updated_at := v_slot_hold.updated_at;
