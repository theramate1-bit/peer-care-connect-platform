...
| Data Processing Agreement (DPA) | `/dpa`                                           | Native: `/dpa`; [`dpaCopy.ts`](../../theramate-ios-client/constants/legal/dpaCopy.ts); static **`dpa.html`** in web deploy output (see root [`vercel.json`](../../vercel.json)); `DPA_URL` | **Deploy** web build + `vercel.json` routes.                          |
| Sub-processors list             | `/subprocessors`                                 | Static **`subprocessors.html`** in web deploy output; `SUBPROCESSORS_URL` in [`config.ts`](../../theramate-ios-client/constants/config.ts)                                                 | Update HTML table when vendors change.                                |
| Acceptable Use Policy (AUP)     | _(often section in Terms or `/acceptable-use`)_  | Not in mobile constants                                                                                                                                                                    | **Counsel:** standalone vs Terms section                              |
